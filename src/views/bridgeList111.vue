/* * @Author: liuhaosheng * @桥梁列表 */
<template>
  <div class="assets-wrap">
    <p class="title-p">
      <span style="display:inline-block;margin-bottom:20px;">>> 桥梁列表</span>
    </p>
    <div class="content">
      <el-row :gutter="0">
        <el-form label-position="right" label-width="80px" :model="searchMap">
          <el-col :span="4">
            <el-form-item label="桥梁" v-if="flage">  
              <!-- v-if="flage"进行桥梁的显示和隐藏 -->
              <el-input v-model="searchMap.T0001_ID" size="small"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="4">
      ...
      <div class="div-btn">
        ...
        <div style="float:right;">
          <el-button type="primary" size="small">全部 602</el-button>
          <el-button type="primary" size="small">警 100</el-button>
          <el-button type="primary" size="small">待检查 230</el-button>
          <el-button type="primary" size="small">已检查 207</el-button>
        </div>
      </div>
      <div class="table-div">
        <el-table
        ...
        >
          ...
          <el-table-column
            ...
            min-width="70"
          ></el-table-column>
          <el-table-column prop="T0002_START_PILE" label="起点桩号" min-width="70"></el-table-column>
          <el-table-column prop="T0002_END_PILE" label="终点桩号" min-width="70"></el-table-column>
          <el-table-column prop="T0002_ASSET_AMOUNT" label="数量（个/座/片）" min-width="70"></el-table-column>
          <el-table-column prop="T0002_LOAD_NAME" label="所属路段" min-width="70"></el-table-column>
          <el-table-column label="图片" align="center" min-width="70">
          ...
          <el-table-column prop="T0002_TECH_STATE" label="技术状况"></el-table-column>
          <el-table-column prop="T0002_ASSET_DATE" label="归属年份" width="120"></el-table-column>
          <el-table-column label="地图位置" min-width="70">
            <template slot-scope="scope">
              <i
      ...
      <el-form :model="addForm" :rules="rules" ref="addFormRef">
        <table class="add-table">
          ...
          <tr>
            ...
            <td style="text-indent:10px;">{{ addForm.T0002_ASSET_COMPANY }}</td>
          ...
          <tr>
            ...
            <td colspan="3">
              <el-input type="textarea" v-model="addForm.T0002_ASSET_REAMRK" maxlength="500"></el-input>
            </td>
          </tr>
        </table>
      </el-form>
    ...
    >
      <el-form :model="editForm" :rules="rules" ref="editFormRef">
        <table class="add-table">
          ...
          <tr>
            <td class="bg-td">归属公司：</td>
            <td>{{ editForm.T0002_ASSET_COMPANY }}</td>
            <td class="bg-td">所属养管公司：</td>
            <td>
              <el-form-item prop="T0002_CURING_UNIT">
                <el-input
    ...
    <!-- 查看 -->
    <el-dialog title=">> 查看资产" :visible.sync="infoShow" custom-class="dialog-div">
      <table class="add-table">
        <tr>
          <td class="bg-td">资产名称：</td>
          <td>{{ infoForm.T0002_ASSET_NAME }}</td>
          <td class="bg-td">资产类别：</td>
          <td>{{ infoForm.T0001_ASSETTYPE_NAME }}</td>
        </tr>
        <tr>
          <td class="bg-td">所属路段：</td>
          <td colspan="3">{{ infoForm.T0002_LOAD_NAME }}</td>
        </tr>
        <tr>
          <td class="bg-td">起点桩号：</td>
          <td>{{ infoForm.T0002_START_PILE }}</td>
          <td class="bg-td">终点桩号：</td>
          <td>{{ infoForm.T0002_END_PILE }}</td>
        </tr>
        <tr>
          <td class="bg-td">数量：</td>
          <td>{{ infoForm.T0002_ASSET_AMOUNT }}</td>
          <td class="bg-td">归属年份：</td>
          <td>{{ infoForm.T0002_ASSET_DATE }}</td>
        </tr>
        <tr>
          <td class="bg-td">归属公司：</td>
          <td>{{ infoForm.T0002_ASSET_COMPANY }}</td>
          <td class="bg-td">所属养管公司：</td>
          <td>{{ infoForm.T0002_CURING_UNIT }}</td>
        </tr>
        <tr>
          <td class="bg-td">责任人：</td>
          <td>{{ infoForm.T0002_DUTY_PERSON }}</td>
          <td class="bg-td">联系电话：</td>
          <td>{{ infoForm.T0002_TOUCH_TEL }}</td>
        </tr>
        <tr>
          <td class="bg-td">经度：</td>
          <td>{{ infoForm.T0002_ASSET_PRECI }}</td>
          <td class="bg-td">纬度：</td>
          <td>{{ infoForm.T0002_ASSET_LATI }}</td>
        </tr>
        <tr>
        ...
        <tr>
          <td class="bg-td">费用况详情（备注）：</td>
          <td colspan="3">{{ infoForm.T0002_ASSET_REAMRK }}</td>
        </tr>
      </table>
    ...
    <!-- 地图 -->
    <el-dialog :visible.sync="mapShow" custom-class="dialog-div" title="地图位置">
      <div style="height: 400px;">
        <el-amap vid="amapDiv" :center="mapData.position" :zoom="mapData.zoom" class="amap-demo">
          <el-amap-marker vid="component-marker" :position="mapData.position" :label="loclabel"></el-amap-marker>
        </el-amap>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    // 点击跳转到checkList
    qlList(row){
      // row.T0002_ID = Number(row.T0002_ID)
      // console.log(row)
      this.$router.push({path:'/checkList',query:{id:row.T0002_ID}})
    },
    // 请求所有的起点 / 终点桩号
    assetDataFun() {
      this.$api.post(
