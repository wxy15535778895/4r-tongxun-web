...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        T0004_CURINGCOST_MONEY: [{ validator: validNum, trigger: 'blur' }],
        T0004_CURINGCOST_TIME: [],
      },
      infoForm: {},
      tableData: [],
      ...
      json_fields: {
        ...
        费用名称: 'T0004_CURINGCOST_NAME',
        '金额（元）': 'T0004_CURINGCOST_MONEY',
        费用发生时间: 'T0004_CURINGCOST_TIME',
        收支选择: 'T0004_CURINGCOST_TYPE',
        '费用况详情（备注）': 'T0004_CURINGCOST_REMARK',
      },
      importFiles: [],
      action:
        this.url +
        '/cycle/curingCost/readCuringCostExcel?M0018_ID=' +
  ...
  methods: {
    ...
    exitReport() {
      this.$api.post(
        '/cycle/curingCost/listAll',
        this.searchMap,
        null,
        (res) => {
          this.allTable = res.data
          this.allTable.forEach((v) => {
            if (v.T0004_CURINGCOST_TYPE == 1) {
              v.T0004_CURINGCOST_TYPE = '收入'
            } else {
              v.T0004_CURINGCOST_TYPE = '支出'
            }
          })
        }
      )
    ...
    uploadModel() {
      window.location.href =
        this.url + '/cycle//uploadFiles/temple/' + 'curingCost.xls'
    },
    // 点击导入
    importExcel() {
