...
export default {
  data() {
    ...
    return {
      ...
      json_fields: {
        ...
        '金额（元）': 'T0005_COSTBUDGET_MONEY',
        养护工程: 'T0005_ENGIN_MAINT',
        工程时间区间开始时间: 'T0005_START_TIME',
        工程时间区间结束时间: 'T0005_END_TIME',
        '费用况详情（备注）': 'T0005_COSTBUDGET_REMARK',
      },
      importFiles: [],
      action:
        this.url +
        '/cycle/costBudget/readCostBudgetExcel?M0018_ID=' +
  ...
  methods: {
    ...
    exitReport() {
      this.$api.post(
        '/cycle/costBudget/listAll',
        this.searchMap,
        null,
        (res) => {
          this.allTable = res.data
          this.allTable.forEach((v) => {
            if (v.T0005_ENGIN_MAINT == 0) {
              v.T0005_ENGIN_MAINT = '日常养护'
            } else if (v.T0005_ENGIN_MAINT == 1) {
              v.T0005_ENGIN_MAINT = '小修保养'
            } else if (v.T0005_ENGIN_MAINT == 2) {
              v.T0005_ENGIN_MAINT = '中修工程'
            } else {
              v.T0005_ENGIN_MAINT = '大修工程'
            }
          })
        }
      )
    ...
    uploadModel() {
      window.location.href =
        this.url + '/cycle//uploadFiles/temple/' + 'costBudget.xls'
    },
    // 点击导入
    importExcel() {
