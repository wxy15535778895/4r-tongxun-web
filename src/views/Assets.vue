...
<template>
  <div id="assets-wrap">
    <p class="title-p" style="display:flex;justify-content:space-between">
    ...
    <div class="content">
      ...
      <div class="div-btn">
        ...
        <download-excel
          ...
          name="资产清单.xls"
        >
          <el-button type="primary" plain size="small" @click="exitReport"
            >导出</el-button
          >
        </download-excel>
      </div>
      ...
      <el-dialog
        ...
        width="40%"
        :before-close="closeDaliog"
      >
        <el-main style="padding: 0 25px !important;">
          <el-form
            ref="importform"
            :model="importform"
          ...
          >
            ...
            >
              <div style="display:flex;flex-direction:column">
                ...
                <div slot="tip" class="el-upload__tip" style="width:120px">
                  只能上传.xls/.xlsx文件
                </div>
              </div>
            </el-upload>
            <p class="mod" style="color:red;padding-top:10px">
              注:导入前请先下载模板！！！
            </p>
          </el-form>
        </el-main>
        <!-- <span slot="footer" class="dialog-footer">
          <el-button
            ...
            >确定</el-button
          >
          <el-button size="small" @click="closeDaliog">取消</el-button>
        </span> -->
      </el-dialog>
      <!-- 错误数据 -->
      <el-dialog
        title="错误数据,请手工录入！"
        :fullscreen="false"
        :visible.sync="wrongMsg"
        width="500px"
        :before-close="closeDaliog"
      >
        <ul
          v-for="(item, index) in wrongMsgList"
          :key="index"
          style="padding-left:10px"
        >
          <li style="height:30px;line-height:30px;font-weight:700;">
            {{ item.errorCol + ',' + item.errorRow + ',' + item.errorMsg }}
          </li>
        </ul>
      </el-dialog>
      <div class="table-div">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="closeDaliog">取 消</el-button>
      </div>
    </el-dialog>
    <!-- 修改 -->
    <el-dialog
      ...
      <table class="add-table">
        ...
        <tr>
          ...
          <td colspan="3">
            ...
            >
              <div
                style="padding:3px 10px;background:skyblue;color:#fff;border-radius:10px"
              >
                附件信息
              </div>
            </el-upload>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        T0002_CURING_UNIT: [],
        T0002_DUTY_PERSON: [],
        T0002_TOUCH_TEL: [{ validator: validTel, trigger: 'blur' }],
      },
      infoForm: {},
      showCount: 10,
      currentPage: 1,
      ...
      allTable: [],
      json_fields: {
        资产名称: 'T0002_ASSET_NAME',
        资产类别: 'T0001_ASSETTYPE_NAME',
        所属路段: 'T0002_LOAD_NAME',
        起点桩号: 'T0002_START_PILE',
        终点桩号: 'T0002_END_PILE',
        数量: 'T0002_ASSET_AMOUNT',
        归属年份: 'T0002_ASSET_DATE',
        归属公司: 'M0018_SIMPLE_NAME',
        所属养管公司: 'T0002_CURING_UNIT',
        责任人: 'T0002_DUTY_PERSON',
        联系电话: 'T0002_TOUCH_TEL',
        经度: 'T0002_ASSET_PRECI',
        纬度: 'T0002_ASSET_LATI',
        费用况详情: 'T0002_ASSET_REAMRK',
      },
      importFiles: [],
      action:
        this.url +
        '/cycle/assetData/readAssetExcel?M0018_ID=' +
  ...
  methods: {
    ...
    searchFun() {
      ...
      this.isSearch = true
      this.getAssetList()
      this.exitReport() // 提前预备导出数据
    },
    // 下载模板
    uploadModel() {
      window.location.href =
        this.url + '/cycle//uploadFiles/temple/' + 'assetData.xls'
    },
    // 点击导入
    importExcel() {
    ...
    getAssetList() {
      ...
      this.$api.post(`/cycle/assetData/listPage`, _data, null, (r) => {
        ...
        for (let i = 0; i < r.data.returnParam.length; i++) {
          if (
            r.data.returnParam[i].files.length > 0 &&
            (r.data.returnParam[i].files[0].FILE_URL.indexOf('.xls') == -1 ||
              r.data.returnParam[i].files[0].FILE_URL.indexOf('.xlsx') == -1)
          ) {
